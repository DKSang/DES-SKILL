---
name: des-semantic-and-serving-layer
...
Use as a routing bridge when a task combines semantic model design and serving layer design. For new work, prefer running `des-semantic-model-design` first, then `des-serving-layer-design` separately.
...
| :--- | :--- |
| Defining metric formulas, KPI hierarchies, dimensions, RLS, or glossary | Use `des-semantic-model-design` |
| Configuring dashboards, APIs, exports, reverse ETL, or AI agent context | Use `des-serving-layer-design` |
...
2. **If metric design is needed**: run `des-semantic-model-design` fully first.
3. **If serving design is needed**: run `des-serving-layer-design` against the completed semantic model.
...
- Semantic model specification (from `des-semantic-model-design`).
- Serving layer specification (from `des-serving-layer-design`).
...
## Certified Metrics (from des-semantic-model-design)
## Serving Channels (from des-serving-layer-design)
## Cross-Check Notes
```
