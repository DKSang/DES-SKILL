...
Use when a request is not yet implementation-ready: new feature, scope change, incident follow-up, optimization idea, governance change, or release blocker. Use before `des-implementation-planning` whenever affected artifacts, downstream consumers, risks, or decision options are unclear.
...
- Recommendation: proceed to `des-implementation-planning`, revise a DES planning artifact, or stop.
...
Use `des-implementation-planning` only after the change brief is accepted. If the brief says a planning artifact is stale, return to the affected phase skill first.
