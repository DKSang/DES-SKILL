---
name: des-implementation-planning
...
4. `step-04-final-validation.md` checks that the plan and implementation story are executable and ready for `des-dev-story`.
...
- If artifacts conflict, HALT and route back to `des-brainstorm-change` or the affected phase skill.
...
Use `des-dev-story` after the plan is marked ready. If readiness fails, return to the named DES phase skill or `des-brainstorm-change`.
