...
Dùng sau `de-architecture-design` và trước khi bắt đầu implementation. Dùng khi:
...
- Business SLAs từ `03-requirements-and-kpis.md`.
...
After writing, summarize path and recommend: nếu có BLOCKER → quay lại `de-architecture-design`; nếu không → tiếp tục `de-ingestion-design`.
...
- Nếu có BLOCKER → quay lại `de-architecture-design` để revise, rồi chạy lại review.
- Nếu chỉ có HIGH/LOW → proceed với `de-ingestion-design`, track HIGH items trong ADR backlog.
