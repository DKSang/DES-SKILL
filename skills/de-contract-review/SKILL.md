...
Dùng sau `de-data-contracts` và trước khi contract được ký kết (hoặc trước khi go-live). Dùng khi:
...
- KPI Catalog (`03-requirements-and-kpis.md`) để cross-check freshness requirements.
...
After writing, summarize path. Nếu có BLOCKER → quay lại `de-data-contracts` để revise. Nếu không → contract ready for signing.
...
- Nếu có BLOCKER → quay lại `de-data-contracts` để revise contract, rồi chạy lại review.
- Nếu chỉ có HIGH → ghi addendum, proceed với `de-transformation-design`.
- Nếu APPROVED → contract ký kết, proceed với `de-data-quality`.
