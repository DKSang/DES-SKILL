...
test("des-ingestion-design uses generic Phase 8 artifact paths and step names", () => {
  ...
  assert.match(customize, /output_file = "\.agents\/des-skill\/output\/08-ingestion-specification\.md"/);
  assert.match(customize, /template_file = "\.agents\/des-skill\/templates\/08-ingestion-specification-template\.md"/);
  assert.match(customize, /checklist_file = "\.agents\/des-skill\/checklists\/08-ingestion-design-checklist\.md"/);
